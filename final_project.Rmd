...
```{r}
# Step 1: Convert 'tracks' column from a string to a list
playlist_subset <- playlist_subset %>%
  mutate(tracks = map(tracks, ~ fromJSON(.) %>% as_tibble()))

# Step 2: Unnest the track details while keeping the playlist name
expanded_df <- playlist_subset %>%
  select(name, tracks) %>%      # Keep playlist name and tracks column
  unnest(tracks)                # Expand song details into separate rows
